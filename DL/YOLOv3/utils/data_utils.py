...
def parse_line(line):
    ...
    if 'str' not in str(type(line)):
        line = line.decode()
    s = line.strip().split(' ')
    assert len(s) > 8, print('%s' % s); 'Annotation error! Please check your annotation file. Make sure there is at least one target object in each image.'
    line_idx = int(s[0])
    pic_path = s[1]
    img_width = int(s[2])
    img_height = int(s[3])
...
def process_box(boxes, labels, img_size, class_num, anchors):
    ...
    try:
        ...
        for i, idx in enumerate(best_match_idx):
            ...
            if x == 52:
                print(ratio)
                print(box_centers[i, 0])
                print(np.floor(box_centers[i, 0] / ratio))
                print(np.shape(y_true_52))
        ...
        print("best_match_idx : %s" % best_match_idx)
        print("idx : %d" % idx)
        print("i : %d" % i)
        print("box_centers : %s" % box_centers)
        print("box_centers[i,:] : %s" % box_centers[i,:])
        print("y_true_len : %d" % len(y_true[feature_map_group]))
        print("y_true : %s" % y_true[feature_map_group][y,:,k,:2])
        print("y, x, k : %d, %d, %d" % (y,x,k))
        print("boxes : %s" % boxes)
        # print("box_centers : %d" % len(y_true[feature_map_group][y, x, k, :2]))
        print(e)
